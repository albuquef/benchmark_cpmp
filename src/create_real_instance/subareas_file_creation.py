...
# voronoi ="" 
grid = ""
# style_decoupage = "grid_" # "voronoi_" "grid_" or "" or "kmeans_"
style_decoupage = "kmeans_"
type_points = "2km"
# type_decoupage = "commune_"  # "Arrondissement" "EPCI" "canton" "commune" "departement" "region"

for type_decoupage in ["Arrondissement_", "EPCI_", "canton_", "commune_"]:
    ...
    if (style_decoupage == ""):
        # eliminate the last _ from the type_decoupage
        type_decoupage = type_decoupage[:-1]
        DECOUPAGE_FILE = f"{style_decoupage}{type_decoupage}"
    ...
    if (style_decoupage == "voronoi_"):
        ...
        if(type_decoupage == "Arrondissement_" or type_decoupage == "EPCI_"):
            ...
            filename = filename[:-1]
            shp_path = f"/home/falbuquerque/Documents/projects/GeoAvignon/Creation_Real_Instance/Decoupages_GIS/decoupages_voronoi/{filename}.shp"
    ...
    if (style_decoupage == "kmeans_"):
        shp_path = f"/home/falbuquerque/Documents/projects/GeoAvignon/Creation_Real_Instance/Decoupages_GIS/decoupages_kmeans/{DECOUPAGE_FILE}.shp"
    ...
    subarea_code = 'FID'
    if (style_decoupage == ""):
        if type_decoupage == "Arrondissement":
            subarea_code = 'codearondo'
        elif type_decoupage == "EPCI":
            subarea_code = 'NOM_EPCI'
        elif type_decoupage == "canton":
            subarea_code = 'numcant'
        elif type_decoupage == "commune":
    ...
    if (len(output_df[output_df["subarea"].isna()]) == 0):
        ...
        if DECOUPAGE_FILE == f"{style_decoupage}Arrondissement_{type_points}" or DECOUPAGE_FILE == f"{style_decoupage}Arrondissement":
            output_df.to_csv(f"outputs/PACA_{type_points}/loc_coverages_{style_decoupage}arrond_{type_points}.txt", sep=" ", index=False)
        elif DECOUPAGE_FILE == f"{style_decoupage}EPCI_{type_points}" or DECOUPAGE_FILE == f"{style_decoupage}EPCI":
            output_df.to_csv(f"outputs/PACA_{type_points}/loc_coverages_{style_decoupage}EPCI_{type_points}.txt", sep=" ", index=False)
        elif DECOUPAGE_FILE == f"{style_decoupage}canton_{type_points}" or  DECOUPAGE_FILE == f"{style_decoupage}canton":
            output_df.to_csv(f"outputs/PACA_{type_points}/loc_coverages_{style_decoupage}canton_{type_points}.txt", sep=" ", index=False)
        elif DECOUPAGE_FILE == f"{style_decoupage}commune_{type_points}" or DECOUPAGE_FILE == f"{style_decoupage}commune":
